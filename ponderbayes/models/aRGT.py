...
class RationalGroupThink(pl.LightningModule):
    ...
    def _shared_step(self, batch, batch_idx, phase):
        ...
        if batch_idx == 0 and self.current_epoch == 0:
            ...
            self.std_err_halted = (
                self.std_err_halted.expand(batch_size, 1).detach().to(self.device)
            )
        ...
        losses_overall = self.total_loss(losses_rec, losses_reg)
        ...
        # (batch_size, 1)
        self.std_err_halted = (
            (std_dev_halted / torch.sqrt(torch.Tensor([self.ensemble_size])))[:, None]
            .detach()
            .to(self.device)
        )
    ...
    def total_loss(self, rec_loss, reg_loss):
        """
        Computes total loss
        """
        if self.anneal_beta:
    ...
    def _loss_step(self, p, y_pred_batch, y_true_batch, lambda_p):
        ...
        # overall loss
        loss_overall = self.total_loss(loss_rec, loss_reg)
    ...
    def beta_annealer(self):
        """
        Returns beta, annealed for the current epoch
        """
        heat = 2
        annealed_beta = (1 / (heat * self.current_epoch + 1 + self.beta)) + self.beta
