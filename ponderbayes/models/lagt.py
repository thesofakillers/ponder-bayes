...
class LambdaGroupThink(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        self.ensemble = nn.ModuleList(
            [
                PonderNetModule(
                    n_elems, n_hidden, max_steps, allow_halting, feed_lambda=True
                )
        ...
        self.std_err = (0.5 / torch.sqrt(torch.Tensor([self.ensemble_size])))[
            :, None
        ].detach()
    ...
    def _shared_step(self, batch, batch_idx, phase):
        ...
        if batch_idx == 0 and self.current_epoch == 0:
            ...
            self.std_err = (
                self.std_err.expand(batch_size, self.max_steps).detach().to(self.device)
            )
        ...
        y_pred_ensemble = torch.zeros(self.ensemble_size, self.max_steps, batch_size)
        ...
        for i, model in enumerate(self.ensemble):
            # forward pass
            y_pred, p, halting_step = model(x, err=self.std_err)
            ...
            with torch.no_grad():
                y_pred_scaled = torch.sigmoid(y_pred).detach()
                y_pred_ensemble[i] = y_pred_scaled
        ...
        std_dev = torch.std(y_pred_ensemble, dim=0).detach()
        # (batch_size, 1)
        self.std_err = (
            (std_dev / torch.sqrt(torch.Tensor([self.ensemble_size])))
            .detach()
            .to(self.device)
        ).T
        std_err_halted = utils.tensor_at_halting_step(
            self.std_err.T, halting_step.long()
        )
        ...
        results = {
            ...
            "std_err": std_err_halted.to("cpu").mean(),
        }
